...
class ViewController: UIViewController {
    ...
    func startGame(){
        ...
        if let savedGameboard = loadTileValueList() {
            tileValueList = savedGameboard.tileValueList
        } else {
            tileValueList = [0]
        }
    ...
    private func saveTileValueList() {
        let isSuccessfulSave = NSKeyedArchiver.archiveRootObject(tileValueList, toFile: GameboardStorage.ArchiveURL.path)
    }
